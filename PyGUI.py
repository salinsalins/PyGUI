...
class MyWidget(Qt.QTabWidget):
    ...
    def __init__(self, parent=None):
        ...
        self._ui.pet10ao00.setRange(0, 10 * ([float(i) for i in (
        tango.AttributeProxy("ET7000_server/test/pet10_7026/ao00").get_property("display_unit")["display_unit"])][0]))
        self._ui.pet10ao01.setRange(0, 10 * ([float(i) for i in (
        tango.AttributeProxy("ET7000_server/test/pet10_7026/ao01").get_property("display_unit")["display_unit"])][0]))
        ...
        self._ui.pet10ao00label.setText(([str(i) for i in (
        tango.AttributeProxy("ET7000_server/test/pet10_7026/ao00").get_property("label")["label"])][0]))
        self._ui.pet10ao01label.setText(([str(i) for i in (
        tango.AttributeProxy("ET7000_server/test/pet10_7026/ao01").get_property("label")["label"])][0]))
    ...
    def valuechangeao00(self):
        a = self._ui.pet10ao00.value()
        bb = taurus.Attribute("pet10/ao00")

        bb.write(a / float(bb.getParam("display_unit")))
        # taurus.Attribute("pet10/ao00").write(a/cc)

    def valuechangeao01(self):
        a = self._ui.pet10ao01.value()
        bb = taurus.Attribute("pet10/ao01")
        bb.write(a / float(bb.getParam("display_unit")))
...
attrs = ["ET7000_server/test/pet10_7026/ao0{}".format(d) for d in range(2)]


# gui(attrs)
def handle_events1(c):
    # e=tango.AttributeProxy("ET7000_server/test/pet10_7026/ao00")
    cc = ([float(i) for i in (tango.AttributeProxy(c.attr_name).get_property("display_unit")["display_unit"])][0])
    text = (c.attr_value.value * cc)

    # b.write(e.attr_value.value)
    a = float(text)
    w._ui.pet10ao00_2.setValue(a)
